...
// This function mutates nthread different demes and evenly distributes the
// number of iterations needed to be done between all of the demes using threads.
// Each thread can update best ordering which is why we implemented std::scoped_lock.
Cities::permutation_t
          ...
          unsigned nthread = 1)
{
  ...
  // Joining the threads together at the end. 
  for (auto& t : threads) {
    t.join();
  }
